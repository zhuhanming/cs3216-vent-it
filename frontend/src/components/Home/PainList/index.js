...
class PainList extends React.Component {
  ...
  sortPainList = () => {
    if (this.state.sortingType === 0) {
      ...
      temp.sort((a, b) =>
        parseInt(a.angry_score) > parseInt(b.angry_score) ? -1 : 1
      );
  ...
  buttonSelector = () => {
    if (this.state.sortingType === 0) {
      return (
        <div className="d-flex flex-row justify-content-center mt-4 pb-2 top-bar-vent">
          <button className="btn btn-vent-2 active mr-2">RAGE</button>
          <button
            className="btn btn-vent-2 ml-2"
    ...
    return (
      <>
        {this.state.painList.map((painPoint) => {
          ...
          let formattedExpiryInDays = Math.ceil(
            formattedExpiry / (1000 * 3600 * 24)
          );
          ...
          return (
            <li
              ...
              onClick={() => this.toggleIndividual(painPoint.id)}
            >
